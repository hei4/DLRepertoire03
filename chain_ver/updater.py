...
class DCGANUpdater(chainer.training.StandardUpdater):
    ...
    def loss_gen(self, gen, y_fake):
        batchsize = len(y_fake)
        loss = F.sum(F.square(y_fake - 1.))     # F.sum(F.softplus(-y_fake)) / batchsize
    ...
    def update_core(self):
        ...
        x_real = Variable(self.converter(batch, self.device)) # Variable(self.converter(batch, self.device)) / 255.
